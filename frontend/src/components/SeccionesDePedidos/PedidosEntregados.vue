<template>
  <div>
    ...
    <div v-if="cargando" class="text-center py-5">
      <div class="spinner-border" :style="{ color: 'var(--colorTerciario)' }" role="status"></div>
    </div>
...
const cargarPedidos = async () => {
  cargando.value = true
  try {
    const q = query(collection(db, 'compras_entregadas'), orderBy('fecha', 'desc'))
    const querySnapshot = await getDocs(q)
    pedidos.value = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))
  } catch (error) {
